...
public:
  ...
  // it makes `eo::Error` constructible from int, and break implicit construction of Result<int> from value.
...
template<>
struct hash<eo::Error> {
  size_t operator()(const eo::Error& err) const noexcept {
...
template<>
struct formatter<eo::Error> {
  constexpr auto parse(format_parse_context& ctx) -> decltype(ctx.begin()) {
    return ctx.begin();
  }

  template<typename FormatContext>
  auto format(const eo::Error& err, FormatContext& ctx) -> decltype(ctx.out()) {
