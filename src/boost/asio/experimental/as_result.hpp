...
#ifndef BOOST_ASIO_EXPERIMENTAL_AS_RESULT_HPP
#define BOOST_ASIO_EXPERIMENTAL_AS_RESULT_HPP
...
namespace asio {
  namespace experimental {
    ...
    /**
     * The as_result_t class is used to indicate that any arguments to the
    ...
    public:
       ...
       * location.
       */
      BOOST_ASIO_CONSTEXPR as_result_t(
        default_constructor_tag = default_constructor_tag(), CompletionToken token = CompletionToken())
        : token_(BOOST_ASIO_MOVE_CAST(CompletionToken)(token)) {}

      /// Constructor.
      template<typename T>
      BOOST_ASIO_CONSTEXPR explicit as_result_t(BOOST_ASIO_MOVE_ARG(T) completion_token)
      ...
      struct executor_with_default : InnerExecutor {
        ...
        executor_with_default(const InnerExecutor& ex) BOOST_ASIO_NOEXCEPT : InnerExecutor(ex) {}

        /// Convert the specified executor to the inner executor type, then use
        /// that to construct the adapted executor.
        template<typename OtherExecutor>
        executor_with_default(const OtherExecutor& ex,
          typename constraint<is_convertible<OtherExecutor, InnerExecutor>::value>::type = 0) BOOST_ASIO_NOEXCEPT
          : InnerExecutor(ex) {}
      };

      /// Type alias to adapt an I/O object to use @c as_result_t as its
      /// default completion token type.
#if defined(BOOST_ASIO_HAS_ALIAS_TEMPLATES) || defined(GENERATING_DOCUMENTATION)
      template<typename T>
      ...
      as_default_on(BOOST_ASIO_MOVE_ARG(T) object) {
        return typename decay<T>::type::template rebind_executor<
          executor_with_default<typename decay<T>::type::executor_type>>::other(BOOST_ASIO_MOVE_CAST(T)(object));
      }
    ...
    /// arguments should be combined into a single argument.
    template<typename CompletionToken>
    BOOST_ASIO_NODISCARD inline BOOST_ASIO_CONSTEXPR as_result_t<typename decay<CompletionToken>::type> as_result(
      BOOST_ASIO_MOVE_ARG(CompletionToken) completion_token) {
      return as_result_t<typename decay<CompletionToken>::type>(
...
#include <boost/asio/experimental/impl/as_result.hpp>

#endif // BOOST_ASIO_EXPERIMENTAL_AS_RESULT_HPP
